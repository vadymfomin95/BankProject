...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleCardNotFoundException(LoanNotFoundException ex) {
        log.error("Loan not found: {}", ex.getMessage());
        return createErrorResponse(ex.getMessage(), HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(LoanExistException.class)
    public ResponseEntity<ErrorResponse> handleCardExistException(LoanExistException ex) {
        log.error("Loan is already exist: {}", ex.getMessage());
